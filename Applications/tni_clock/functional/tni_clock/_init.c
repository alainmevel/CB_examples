...
#include "tni_clock/functional/tni_clock/tni_clock.h"
...
{
	tni_clock_B1 = self.Bool;
	self.Bool++;
	tni_clock_B2 = self.Bool;
	self.Bool++;
	tni_clock_B3 = self.Bool;
	self.Bool++;
	tni_clock_B4 = self.Bool;
	self.Bool++;
	tni_clock_depth = self.Int;
	self.Int++;
}
...
int behaviorsInit(void)
{
	...
	tni_clock__num = self_num;
	inputOutputVariablesInit();
	_tni_clock__tni_clock_init();
...
void variablesInitialization(void)
{
	...
	CB_Char_Init((char *) memHeader, 0 + 8, 'l');
	CB_Char_Init((char *) memHeader, 0 + 9, 'o');
	CB_Char_Init((char *) memHeader, 0 + 10, 'c');
	CB_Char_Init((char *) memHeader, 0 + 11, 'k');
	...
	CB_Char_Init((char *) memHeader, 53 + 0, 0); /*LCD/displayString size1*/
	CB_Char_Init((char *) memHeader, 53 + 1, 8); /*LCD/displayString size2*/
}
