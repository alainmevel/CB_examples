...
{
	bottle_pulseGenerator = self.Bool;
	self.Bool++;
	bottle_loading = self.Bool;
	self.Bool++;
	bottle_evacuation = self.Bool;
	self.Bool++;
	bottle_inputFlow = self.Float;
	self.Float++;
	bottle_bottleFilled = self.Bool;
	self.Bool++;
	bottle_bottlePresent = self.Bool;
	self.Bool++;
}
...
void variablesInitialization(void)
{
	...
	CB_Char_Init((char *) memHeader, 0 + 8, 'e');
