...
#include "timers_counters/functional/cb_ctd/cb_ctd.h"
...
int behaviorsInit(void)
{
	...
	cb_ctd__num = self_num;
	inputOutputVariablesInit();
	_timers_counters__cb_ctd_init();
...
void variablesInitialization(void)
{
	...
	CB_Char_Init((char *) memHeader, 0 + 4, 'b');
	CB_Char_Init((char *) memHeader, 0 + 5, '_');
	CB_Char_Init((char *) memHeader, 0 + 6, 'c');
	CB_Char_Init((char *) memHeader, 0 + 7, 't');
	CB_Char_Init((char *) memHeader, 0 + 8, 'd');
