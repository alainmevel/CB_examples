...
static CB_Mem_Bool *faults_restCmd;
...
#define restCmd (faults_restCmd->CB_previous_value)		/* Rest command to the valve */
...
label2:	/* Faults of movements */
	...
	if(((ton_sec_2_Q = ton_sec_Q(restCmd, presetMvt, &ton_sec_2_i_ton, &ton_sec_2_t1_ton, &ton_sec_2_t2_ton)) & !inletSensor)) {
		inletSensorFault = 1;
	}
label3:	/* Discordance fault */
	if((ton_ms_3_Q = ton_ms_Q((inletSensor & outletSensor), presetDiscordance, &ton_ms_3_i_tonms, &ton_ms_3_t1_tonms, &ton_ms_3_t2_tonms))) {
		discordanceFault = 1;
	}
