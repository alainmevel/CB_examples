...
#define autoCmd (actuator_autoCmd->CB_previous_value)		/* Automatic command */
...
#define cmd (actuator_cmd->CB_current_value)		/* Agitator command */
...
label1:	/* Forward command */
	cmd = (((autoMode & autoCmd) | (((manualMode & forwardPB) | (manualMode & cmd)) & !stopPB)) & !generalFault);
label2:
label3:
