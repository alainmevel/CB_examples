...
static int cmd_equipment_library__actuator(void)
{
	...
	if (exec_term_node(self_num)) {
...

		{
			...
			cmd = _8;
			cov_output(__comp_name, self_num, 5, 0, cmd);
		}
...
static int cmd_equipment_library__faults(void)
{
	...
	if (exec_term_node(self_num)) {
...

		{
			...
			cov_output(__comp_name, self_num, 3, 0, onFBFault);
			if (_1) {
				generalFault = 0;
			}
			...
			cov_output(__comp_name, self_num, 8, 0, onFBFault);
			_12 = !thermalFB;
			cov_output(__comp_name, self_num, 12, 0, _12);
			if (_12) {
				thermalFault = 1;
			}
			cov_output(__comp_name, self_num, 11, 0, thermalFault);
			_16 = thermalFault;
...
int cmd_equipment_library__cmd_on_off_pump(void)
{
	CB_Object __start = self;
	int __ret;
	static char *__comp_name = "cmd_equipment_library.cmd_on_off_pump";
