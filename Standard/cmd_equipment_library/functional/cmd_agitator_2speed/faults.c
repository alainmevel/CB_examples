...
static CB_Mem_Bool *faults_highSpeedFBFault;
static CB_Mem_Bool *faults_lowSpeedFBFault;
...
#define thermalFB (faults_thermalFB->CB_previous_value)		/* Feedback of the thermal protection of the agitator */
#define acknowledgePB (faults_acknowledgePB->CB_previous_value)		/* Acknowledgement pushbutton */
#define generalFault (faults_generalFault->CB_current_value)		/* General fault of the agitator */
...
#define preset (*(long *)(&(faults_preset->CB_current_value)))		/* in seconds */
#define thermalFault (faults_thermalFault->CB_current_value)		/* Thermal (electric) fault */
#define highSpeedFBFault (faults_highSpeedFBFault->CB_current_value)		/* High speed feedback fault */
#define lowSpeedFBFault (faults_lowSpeedFBFault->CB_current_value)		/* Low speed feedback fault */
...
******************************************************/
	...
	faults_highSpeedFBFault = self.Bool + 4;
	faults_lowSpeedFBFault = self.Bool + 5;
...
label2:	/* Fault of forward feedback */
	if(((ton_sec_1_Q = ton_sec_Q(highSpeedCmd, preset, &ton_sec_1_i_ton, &ton_sec_1_t1_ton, &ton_sec_1_t2_ton)) & !highSpeedFB)) {
		highSpeedFBFault = 1;
	}
	if(((ton_sec_2_Q = ton_sec_Q(lowSpeedCmd, preset, &ton_sec_2_i_ton, &ton_sec_2_t1_ton, &ton_sec_2_t2_ton)) & !lowSpeedFB)) {
		lowSpeedFBFault = 1;
	}
