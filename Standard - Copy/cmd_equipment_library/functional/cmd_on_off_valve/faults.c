...
static CB_Mem_Bool *faults_closeCmd;
static CB_Mem_Bool *faults_openCmd;
static CB_Mem_Bool *faults_closedSensorFB;
static CB_Mem_Bool *faults_openedSensorFB;
...
static CB_Mem_Bool *faults_closedSensorFault;
static CB_Mem_Bool *faults_openedSensorFault;
...
/*****************************************************
                             Variables
******************************************************/
#define closeCmd (faults_closeCmd->CB_previous_value)		/* Close command to the valve */
#define openCmd (faults_openCmd->CB_previous_value)		/* Open command to the valve */
#define closedSensorFB (faults_closedSensorFB->CB_previous_value)		/* Closed sensor feedback */
#define openedSensorFB (faults_openedSensorFB->CB_previous_value)		/* Opened sensor feedback */
#define acknowledgePB (faults_acknowledgePB->CB_previous_value)		/* Acknowledgement pushbutton */
#define generalFault (faults_generalFault->CB_current_value)		/* General fault of the valve */
...
#define closedSensorFault (faults_closedSensorFault->CB_current_value)		/* Closed sensor fault */
#define openedSensorFault (faults_openedSensorFault->CB_current_value)		/* Opened sensor fault */
...
******************************************************/
	...
	faults_ton_sec_1_i_ton = self.Int + 9;
	faults_ton_sec_1_t1_ton = self.Float + 3;
	faults_ton_sec_1_t2_ton = self.Int + 10;
	faults_ton_sec_1_Q = self.Bool + 3;
	faults_ton_sec_1_CV = self.Int + 11;
	faults_ton_sec_2_i_ton = self.Int + 12;
	faults_ton_sec_2_t1_ton = self.Float + 4;
	faults_ton_sec_2_t2_ton = self.Int + 13;
	faults_ton_sec_2_Q = self.Bool + 4;
	faults_ton_sec_2_CV = self.Int + 14;
	faults_ton_ms_3_i_tonms = self.Int + 15;
	faults_ton_ms_3_t1_tonms = self.Float + 5;
	faults_ton_ms_3_t2_tonms = self.Int + 16;
	faults_ton_ms_3_Q = self.Bool + 5;
	faults_ton_ms_3_CV = self.Int + 17;
	faults_presetMvt = self.Int + 18;
	faults_presetDiscordance = self.Int + 19;
	faults_closedSensorFault = self.Bool + 6;
	faults_openedSensorFault = self.Bool + 7;
	faults_discordanceFault = self.Bool + 8;
...
label2:	/* Faults of movements */
	if(((ton_sec_1_Q = ton_sec_Q(openCmd, presetMvt, &ton_sec_1_i_ton, &ton_sec_1_t1_ton, &ton_sec_1_t2_ton)) & !openedSensorFB)) {
		openedSensorFault = 1;
	}
	if(((ton_sec_2_Q = ton_sec_Q(closeCmd, presetMvt, &ton_sec_2_i_ton, &ton_sec_2_t1_ton, &ton_sec_2_t2_ton)) & !closedSensorFB)) {
		closedSensorFault = 1;
	}
label3:	/* Discordance fault */
	if((ton_ms_3_Q = ton_ms_Q((closedSensorFB & openedSensorFB), presetDiscordance, &ton_ms_3_i_tonms, &ton_ms_3_t1_tonms, &ton_ms_3_t2_tonms))) {
		discordanceFault = 1;
	}
